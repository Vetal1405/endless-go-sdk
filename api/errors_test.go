...
func Test_Error(t *testing.T) {
	...
	testJson := fmt.Sprintf(`{"message": "%s","error_code": "%s"}`, errorMessage, errorCode)
	data := &Error{}
...
func Test_ErrorWithVm(t *testing.T) {
	...
	testJson := fmt.Sprintf(`{"message": "%s","error_code": "%s","vm_error_code": %d}`, errorMessage, errorCode, vmErrorCode)
	data := &Error{}
	err := json.Unmarshal([]byte(testJson), &data)
	assert.NoError(t, err)
	assert.Equal(t, errorMessage, data.Message)
